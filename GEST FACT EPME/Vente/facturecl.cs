...
{
    public partial class factclibc : System.Windows.Forms.Form
    {
            ...
            if (Tcodc.Text != "")
            {
                if (cmbtypevente.SelectedIndex != -1)
                {
                    if (!dated.Text.Equals("") && !datef.Text.Equals(""))
                    {
                        ...
                        string req20 = "select num,date,totalttc from eentl where codes = '" + Program.Societe + "' and codee = '" + Program.Exercice + "' and codem = '" + Program.Magasin + "'   and date>='" + date1 + "' AND date<='" + date2 + "' and codec= '" + Tcodc.Text + "' and facture = false ";
                        ...
                        if (ds != null)
                        {
                            if (ds.Tables.Count != 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    Total();
                                    MessageBox.Show("Pas de BL à Facturer");
                                }
                            }
                ...
                if (dr.Cells["choix"].Value != null)
                    if ((Boolean)dr.Cells["choix"].Value)
                    {
                        string sql = "INSERT INTO nblfacture (`codes`, `codee`, `codem`, `NUMF`, `NUML`) VALUES ("
                                         ...
                                         + ")";
                        met.Execute(sql);
                        string sqlmodif = "UPDATE eentl SET facture = true   where codes='" + Program.Societe + "' and codee = '" + Program.Exercice + "' and codem = '" + Program.Magasin + "' and num = '" + dr.Cells["num"].Value + "'";
