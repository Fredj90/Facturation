...
{
    public partial class facturecbd : System.Windows.Forms.Form
    {
        ...
        private void LoadB()
        {
            ...
            if (!f)
            {
                ...
                string reqcl = "Select * From eentcb where codes='" + Program.Societe + "' and codee='" + Program.Exercice + "' and num = '" + BNum + "'";
                DataSet dscli = met.recuperer_table(reqcl, "eentcb");
                if (dscli != null)
                    if (dscli.Tables.Count != 0)
                        if (dscli.Tables["eentcb"].Rows.Count != 0)
                        {
                            Tnomc.Text = dscli.Tables["eentcb"].Rows[0].Field<object>("nomc") + "";
                            Tcodc.Text = dscli.Tables["eentcb"].Rows[0].Field<object>("codec") + "";
                            Tadrc.Text = dscli.Tables["eentcb"].Rows[0].Field<object>("adrc") + "";
            ...
            {
                    {
                        string sql = "INSERT INTO nblfacture (`codes`, `codee`, `codem`, `NUMF`, `NUML`) VALUES ("
                                         ...
                                         + ")";
                        met.Execute(sql);
                        string sqlmodif = "UPDATE eentcb SET facture = true   where codes='" + Program.Societe + "' and codee = '" + Program.Exercice + "' and codem = '" + Program.Magasin + "' and num = '" + BNum + "'";
