...
{
    public partial class FCAvance : DevComponents.DotNetBar.Office2007Form
    {
       public MetierClient cli=null;
            ...
            if (cli != null)
            {
                if (!modif)
                {
                    ...
                    if (err.Equals(""))
                    {
                        ...
                        sql = "UPDATE contrat set Favance = true, NUMFAvance='" + met.CString(nfact.Text) + "' WHERE ID = " + IDContrat;
                        met.Execute(sql);
                        //Montage
        ...
        private void FCAvance_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                double.TryParse(dr["THT"] + "", out xtht);
                double.TryParse(dr["TVA"] + "", out xtva);
                double.TryParse(dr["TTC"] + "", out xttc);
                double.TryParse(dr["VAVANC"] + "", out xav);
                ...
                mygrid1.Rows.Add();
                {
                    ...
                    drr.Cells["Npylon"].Value = "Avance Sur Marché de sous traitance EPME/" + Tnomc.Text + " du " + ((DateTime)dr["DATEDEBUT"]).ToString("dd/MM/yyyy") + "";
