...
{
    public partial class FCFondation : DevComponents.DotNetBar.Office2007Form
    {
       public MetierClient cli=null;
            ...
            if (cli != null)
            {
                if (!modif)
                {
                    ...
                    if (err.Equals(""))
                    {
                        ...
                        //Fondation
                        String sqlF = "insert INTO lfondation (NPYLONE,TYPEPYLONE,Tmassif,VBETPROPRETE,VB350,VTOTAL,U,PUHT,PTHT,TVA,CODES,codee,NUMF,IDLC) values";
                        Boolean frst = true;
                        foreach (DataGridViewRow dr in mygrid1.Rows)
                        {
                            if (!dr.IsNewRow)
        ...
        {
            rpt.frm_fondation frm = new rpt.frm_fondation();
            frm.numf = nfact.Text;
            frm.ShowDialog();
        }
