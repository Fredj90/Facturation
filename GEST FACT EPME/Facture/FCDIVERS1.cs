...
{
    public partial class FCDIVERS : DevComponents.DotNetBar.Office2007Form
    {
       ...
       public FCDIVERS()
        ...
        private void buttonX2_Click(object sender, EventArgs e)
        {
            if (cli != null)
            {
                if (!modif)
                {
                    ...
                    {
                        decimal x15=0, x50 = 0;
                        decimal.TryParse(rs15.Text, out x15);
                        decimal.TryParse(rs50.Text, out x50);
                        ...
                        string sql = "INSERT INTO Facture (NUMF,REF,NREF,VREF,CODEC, NOMC,ADRC,DATEF, CODES,CODEE, THT,RAVANCE,RGARANTIE,NETHT, TVA,TIMBRE,TTC,RS50,RS15,NET,MODEP,type,VGARANTIE,VAVANCE,prorata,Contrat) VALUE ("
                            ...
                            + "'" + x50.ToString().Replace(Program.sep, string.Empty) + "',"
                            + "'" + x50.ToString().Replace(Program.sep, string.Empty) + "',"
                        ...
                        //Montage
                        String sqlF = "insert INTO ldivers (NPYLONE,TYPEPYLONE,QTE,U,PUHT,PTHT,TVA,CODES,CODEE,NUMF,IDLC) values";
                        Boolean frst = true;
                        foreach (DataGridViewRow dr in mygrid1.Rows)
                        {
                            if (!dr.IsNewRow)
        ...
        {
            //String sql = "UPDATE lcontrat set proratam = proratam + " + tprorata.Text + " WHERE ID = " + ID;
           // met.Execute(sql);
        }
                   ...
                   foreach (string  str in LSTid)
                   {
                       ...
                       if(dsdd.Tables.Count!=0)
                           if (dsdd.Tables[0].Rows.Count != 0)
                           {
                               ...
                               mygrid1.Rows[i].Cells["VBP"].Value = 0;
                               mygrid1.Rows[i].Cells["U"].Value = "U";
                               mygrid1.Rows[i].Cells["PUHTF"].Value = 0;
