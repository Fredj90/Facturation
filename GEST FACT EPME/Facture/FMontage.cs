...
{
    public partial class FMontage : DevComponents.DotNetBar.Office2007Form
    {
            ...
            if (cli != null)
            {
                if (!modif)
                {
                    ...
                    if (err.Equals(""))
                    {
                        ...
                        //Montage
                        String sqlF = "insert INTO lmontage (NPYLONE,TYPEPYLONE,POIDS,U,PUHT,PTHT,TVA,CODES,CODEE,NUMF) values";
                        Boolean frst = true;
                        foreach (DataGridViewRow dr in mygrid1.Rows)
                        {
                            if (!dr.IsNewRow)
                ...
                else
                {
                    ...
                    if (err.Equals(""))
                    {
                        ...
                        string sql = "UPDATE Facture SET REF='" + met.CString(tref.Text)
                            ...
                            + "',type='M',VGARANTIE='" + Tpret.Text.Replace(Program.sep, string.Empty)
                        ...
                        //Fondation
                        met.Execute("DELETE  FROM lmontage WHERE codes='" + Program.Societe + "' and codee='" + Program.Exercice + "' and numf='" + NumFact + "'");
                        String sqlF = "INSERT INTO lmontage (NPYLONE,TYPEPYLONE,POIDS,U,PUHT,PTHT,TVA,CODES,CODEE,NUMF) values";
                        Boolean frst = true;
                        foreach (DataGridViewRow dr in mygrid1.Rows)
                        {
           ...
           else
           {
               ...
               try {
                   ...
                   foreach (DataRow dr in dsd.Tables[0].Rows)
                   {
                       ...
                       mygrid1.Rows[i].Cells["VBP"].Value = dr["POIDS"];
